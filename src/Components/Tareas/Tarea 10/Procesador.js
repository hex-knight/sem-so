...
export default class Procesador extends PureComponent {
    ...
    checkMemory = () => {
        ...
        var avMemory = 4;
        var time = this.state.contadorGlobal;
    ...
    logKey = (e) => {
        switch (e.code) {
            ...
            case 'KeyN':
                ...
                break;
            case 'KeyT':
                if (this.state.complete === false && this.state.showTable === false) {
                    this.showTable();
                }
                break;
            default:
    ...
    checkQuantum = () =>{
        var quantum = this.state.quantum;
        var progresoActual = this.state.progresoActual;
        if( progresoActual === quantum ){
            this.reEnqueueProcess();
        }
    }
    ...
    reEnqueueProcess = () =>{
        ...
        if (procesoActual) {
            ...
            this.setState({ listos, progresoActual: 0 });
            this.getProces();
    ...
    showTable = () => {
        this.setState({ loading : true })
        setTimeout(() => {
            this.setState({ pause: true, showTable: true, loading: false })
        }, 10);
    }
    ...
    closeTable = () => {
        this.setState({ loading : true })
        setTimeout(() => {
            this.setState({ pause: false, showTable: false, loading: false })
        }, 10);
    }
    ...
    createNew = () => {
        ...
        if (i === false) {
            this.setState({ nuevos, loading: false });
        }
    }
    ...
    renderProcesoActual = () => {
        var procesoActual = this.state.procesoActual;
        var tiempoTranscurrido = Math.ceil(this.state.progresoActual)+ procesoActual ? (procesoActual.ttot - procesoActual.time) : 0;
        var listos = this.state.listos;
        var nuevos = this.state.nuevos;
        var bloqueados = this.state.procesosBloqueados;
                        ...
                        <Col xs={16} sm={16} md={16} lg={12} xl={12} >
                            <Progress type="circle"
                                ...
                                percent={
                                    procesoActual !== null ?
                                        Math.ceil(
                                            (this.state.progresoActual+(procesoActual.ttot - procesoActual.time) * 100) /
                                            procesoActual.ttot) :
                                        0
                                } />
                        </Col>
    ...
    getPreliminarArray = () => {
        ...
        var salida = [];
        if(nuevos){
            salida = salida.concat(nuevos);
        }
        if(listos){
            for(var i = 0; i < listos.length; i++){
                salida.push(this.getPreData(listos[i]));
            }
        }
        if(procesoActual){
            salida.push(this.getPreData(procesoActual));
        }
        if(bloqueados){
            for(var j = 0; j < bloqueados.length; j++){
                salida.push(this.getPreData(bloqueados[j]));
            }
        }
        if(terminados){
            salida = salida.concat(terminados)
        }
        return salida;
    }

    renderPreliminar = () => {
        var table = this.getPreliminarArray();
        return this.state.showTable ? <Modal
                    title="Datos Preliminares"
                    centered
                    visible={this.state.showTable}
                    onOk={() => this.closeTable()}
